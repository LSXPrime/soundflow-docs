...
<Tabs color="primary" variant="bordered" aria-label="Advanced Topics">
    ...
    >
        ...
        *   **Security Components**: Implement custom watermarking, encryption, or acoustic fingerprinting logic using the new `SoundFlow.Security` framework.
        ...
        *   **Extensions (e.g., for specific DSP libraries)**: SoundFlow supports integration with external audio processing libraries. For instance, the `SoundFlow.Extensions.WebRtc.Apm` package provides features like noise suppression and echo cancellation by wrapping the WebRTC Audio Processing Module. You can create similar extensions for other libraries.
        ...
        You can add support for new audio formats by implementing the `ICodecFactory` interface. The engine maintains a list of registered factories and queries them based on priority when a decoder or encoder is requested for a specific format ID.
        ...
        **Usage:**
        ...
        <Steps layout='vertical'>
            ...
            <Step title="Implement MIDI Control (Optional)" icon='mingcute:midi-fill'>
        ...
        **Example (Transpose):**
        ...
        <Steps layout='vertical'>
            ...
            <Step title="Implement ProcessOnAudioData" icon='carbon:data-vis-4'>
                This method receives a `ReadOnlySpan<float>` containing audio data that should not be modified. You should process this data and store the relevant information needed for rendering.
            </Step>
        ...
        **Example (Skeleton):**
        ...
        SoundFlow v1.4 is fully compatible with NativeAOT compilation, enabling you to build self-contained, high-performance applications with minimal startup time and memory footprint. However, because NativeAOT performs aggressive trimming to remove unused code, you must inform the compiler about any custom types that are used dynamically, such as when loading a project file.

        <Steps layout='vertical'>
            <Step title="Understand the Challenge" icon='ph:question-bold'>
                When you save a `Composition` to a `.sfproj` file, the names of your custom `SoundModifier`, `AudioAnalyzer`, and `MidiModifier` classes are stored as strings. During loading, SoundFlow needs to find these types by name to recreate them. In a standard .NET application, this is done with reflection (`Type.GetType("MyTypeName")`), but the AOT compiler cannot know which types are needed and may trim them from the final executable.
            </Step>
            <Step title="Register Your Custom Types" icon='material-symbols:app-registration-outline'>
                To solve this, SoundFlow provides a `TypeRegistry`. At your application's startup, you must explicitly register any custom types that you want to be available for project deserialization. This creates a strong reference, preventing the AOT compiler from removing them.
            </Step>
        </Steps>

        **Example:**

        If you have created a custom modifier called `BitCrusherModifier`, you must register it when your application starts:
        ...
        public static class Program
        {
            ...
            {
                // Register custom types before loading any projects.
                TypeRegistry.RegisterType<BitCrusherModifier>();
                TypeRegistry.RegisterType<MyCustomAnalyzer>();
        ...
        By doing this, `CompositionProjectManager.LoadProjectAsync` will be able to correctly find and instantiate your custom components, even in a fully AOT-compiled and trimmed application.
    </Tab>
    ...
    >
        ...
        *   **SIMD:** SoundFlow uses SIMD instructions (when available) in critical paths like the `Mixer`, `SoundComponent` (for volume and panning), `MathHelper` (for FFTs and windowing), the `ChannelMixer` utility, and the `WsolaTimeStretcher`. Ensure your target platform supports SIMD for the best performance.
        *   **Time-Stretching Quality:** When using the `PlaybackSpeed` property on players, you can balance audio quality and CPU usage. Use `player.SetTimeStretchQuality(WsolaPerformancePreset)` with options like `Fast`, `Balanced` (default), or `HighQuality` to tune the internal WSOLA algorithm for your needs. Lower quality settings consume significantly less CPU, which is ideal for less critical applications or speech.
