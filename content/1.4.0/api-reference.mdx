...
*   **`SoundFlow.Security`:** Contains high-level classes for audio encryption, digital signatures, watermarking, and acoustic fingerprinting.
*   **`SoundFlow.Security.Analyzers`:** Contains `AudioAnalyzer` implementations for extracting or verifying security features like watermarks and fingerprints.
*   **`SoundFlow.Security.Configuration`:** Contains configuration classes for all security-related operations.
*   **`SoundFlow.Security.Containers`:** Contains classes for handling the secure audio container format.
*   **`SoundFlow.Security.Models`:** Contains data models used by the acoustic fingerprinting system.
*   **`SoundFlow.Security.Modifiers`:** Contains `SoundModifier` implementations for embedding security features like watermarks and applying encryption.
*   **`SoundFlow.Security.Payloads`:** Contains interfaces and classes for defining watermark payloads.
*   **`SoundFlow.Security.Stores`:** Contains interfaces and reference implementations for fingerprint storage.
*   **`SoundFlow.Security.Utils`:** Contains utility classes for security operations like key generation.
...
| [`WsolaTimeStretcher`](#components-wsolatimestretcher)	 | A highly configurable, SIMD-optimized component that implements the WSOLA algorithm for real-time, pitch-preserved time stretching. |
| [`WsolaConfig`](#components-wsolaconfig)                       | A configuration container for the `WsolaTimeStretcher`'s internal parameters.                                                                            |
| [`WsolaPerformancePreset`](#components-wsolaperformancepreset) | An enum defining quality and performance presets for the `WsolaTimeStretcher`.                                                                         |
...
| Class/Interface                                                        | Description                                                                                                                            |
| ---------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| [`CompositionProjectManager`](#editingpersistence-compositionprojectmanager) | Static class for saving and loading `Composition` projects to/from `.sfproj` files. Handles media consolidation, relinking, and digital signing. |
| [`ProjectSaveOptions`](#editingpersistence-projectsaveoptions)           | Provides configurable options for saving a project, such as media consolidation, embedding, signing, and folder names.                          |
...
| [`LogLevel`](#enums-loglevel)                    | Defines severity levels for log messages (Debug, Info, Warning, Error, Critical).                                                                     |
...
| [`FilterType`](#enums-filtertype)                | Specifies the type of filter to use in the `Filter` and `ParametricEqualizer` components (Peaking, LowShelf, HighShelf, LowPass, HighPass, BandPass, Notch, AllPass). |
...
| Class/Interface                                                           | Description                                                                                                                                     |
|---------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------|
| [`AudioEncryptor`](#security-audioencryptor)                              | Static class providing high-level methods to encrypt and decrypt audio streams using a secure container format.                                 |
| [`FileAuthenticator`](#security-fileauthenticator)                        | Static class for creating and verifying ECDSA digital signatures for files and streams, ensuring integrity and authenticity.                  |
| [`AudioWatermarker`](#security-audiowatermarker)                          | Static class providing high-level methods to embed and extract robust ownership watermarks in audio.                                          |
| [`AudioIdentifier`](#security-audioidentifier)                            | Static class for generating acoustic fingerprints and identifying audio by matching against a fingerprint store.                                |
| [`WatermarkTuner`](#security-watermarktuner)                              | Static utility to automatically find the optimal watermarking configuration for a given audio source and payload.                             |
| **Security.Analyzers Namespace**                                          |                                                                                                                                                 |
| [`ContentFingerprintAnalyzer`](#securityanalyzers-contentfingerprintanalyzer) | An `AudioAnalyzer` that generates a robust acoustic fingerprint from an audio stream using spectral analysis.                               |
| [`IntegrityWatermarkVerifyAnalyzer`](#securityanalyzers-integritywatermarkverifyanalyzer) | An `AudioAnalyzer` that verifies a fragile, block-chained integrity watermark and raises an event on tampering detection.         |
| [`OwnershipWatermarkExtractAnalyzer`](#securityanalyzers-ownershipwatermarkextractanalyzer) | An `AudioAnalyzer` that extracts a hidden ownership watermark from an audio stream.                                                       |
| **Security.Configuration Namespace**                                      |                                                                                                                                                 |
| [`EncryptionConfiguration`](#securityconfiguration-encryptionconfiguration) | Configuration class for stream encryption, containing the encryption key and IV.                                                              |
| [`FingerprintConfiguration`](#securityconfiguration-fingerprintconfiguration) | Configuration class for acoustic fingerprinting, controlling parameters like FFT size, frequency range, and match confidence.         |
| [`SignatureConfiguration`](#securityconfiguration-signatureconfiguration)   | Configuration class for digital signatures, containing the PEM-encoded private and public keys.                                               |
| [`WatermarkConfiguration`](#securityconfiguration-watermarkconfiguration)     | Configuration class for watermarking, controlling parameters like the secret key, embedding strength, and spread factor.            |
| **Security.Modifiers Namespace**                                          |                                                                                                                                                 |
| [`StreamEncryptionModifier`](#securitymodifiers-streamencryptionmodifier)       | A `SoundModifier` that performs real-time AES-256-CTR encryption or decryption on an audio stream.                                      |
| [`IntegrityWatermarkEmbedModifier`](#securitymodifiers-integritywatermarkembedmodifier) | A `SoundModifier` that embeds a fragile integrity watermark using block-chained LSB steganography.                                      |
| [`OwnershipWatermarkEmbedModifier`](#securitymodifiers-ownershipwatermarkembedmodifier) | A `SoundModifier` that embeds a robust ownership watermark using Direct Sequence Spread Spectrum (DSSS).                                |
| **Security.Stores Namespace**                                             |                                                                                                                                                 |
| [`IFingerprintStore`](#securitystores-ifingerprintstore)                  | Defines the contract for a fingerprint storage backend (e.g., a database).                                                                    |
| [`InMemoryFingerprintStore`](#securitystores-inmemoryfingerprintstore)        | A reference implementation of `IFingerprintStore` that stores fingerprints in memory.                                                       |
| **Other Security Types**                                                  |                                                                                                                                                 |
| `IWatermarkPayload`                                                       | Interface defining a contract for data that can be embedded as a watermark payload.                                                             |
| `TextPayload`                                                             | An implementation of `IWatermarkPayload` for embedding simple text strings.                                                                   |
| `SignatureKeyGenerator`                                                   | A static utility for generating new ECDSA key pairs for use with `SignatureConfiguration`.                                                  |
...
| Class                                           | Description                                                                                                                                             |
|-------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|
| [`BiquadFilter`](#utils-biquadfilter)           | A standalone, single-channel biquad filter implementation with coefficient calculation for various filter types.                                            |
...
| [`TypeRegistry`](#utils-typeregistry)           | A registry for resolving types by name at runtime, ensuring NativeAOT compatibility for project deserialization.                                        |
...
*   `Process(ReadOnlySpan<float> buffer, int channels)`: Processes the audio data, calling `Analyze` and then sending data to the attached visualizer.
*   `Analyze(ReadOnlySpan<float> buffer, int channels)`: Abstract method to be implemented by derived classes to perform audio analysis on the provided buffer.
...
public abstract class SoundPlayerBase : SoundComponent, ISoundPlayer
{
    ...
    public WsolaConfig TimeStretchConfig { get; set; }
...
*   `TimeStretchConfig`: Gets or sets the detailed configuration for the internal WSOLA time stretcher. Allows fine-tuning of window size, hop size, and search radius for advanced users.
...
*   `SetTimeStretchQuality(WsolaPerformancePreset preset)`: A convenient method to configure the time stretcher's quality and performance using a predefined preset.
...
### Components `WsolaPerformancePreset`
...
```
**Description:** Defines performance presets for the `WsolaTimeStretcher`. These presets provide an easy way to balance between CPU usage, latency, and audio quality.
*   `Fast`: Optimized for low latency and low CPU usage. Suitable for speech or when performance is critical.
*   `Balanced`: The standard configuration offering a good trade-off between quality and performance. The default for `SoundPlayerBase`.
*   `HighQuality`: Optimized for smoother audio stretching with fewer artifacts, at the cost of higher latency and CPU usage.
*   `Audiophile`: Maximum quality configuration for complex polyphonic material, using the most CPU resources.

### Components `WsolaConfig`
...
```
**Description:** A configuration container for the `WsolaTimeStretcher`'s internal parameters, allowing for advanced, fine-grained control over the time-stretching algorithm.

**Properties:**
*   `WindowSizeFrames`: The length of the analysis window in frames. Larger values can improve quality for low frequencies but increase latency.
*   `AnalysisHopFrames`: The hop size for analysis.
*   `SearchRadiusFrames`: The range of frames to search for the best overlap match. Larger values can reduce artifacts but increase CPU load.

### Components `WsolaTimeStretcher`
...
```
**Description:** Implements the WSOLA (Waveform Similarity Overlap-Add) algorithm for real-time, pitch-preserved time stretching of audio. This highly-optimized component uses unsafe pointer arithmetic and SIMD instructions for maximum performance. It is used internally by `SoundPlayerBase` and `AudioSegment`.

**Methods:**
*   `Configure(WsolaConfig config)`: Updates the internal configuration parameters. This is a heavy operation that will clear buffers and reset the processing state.
*   `Configure(WsolaPerformancePreset preset)`: A convenient method to update the configuration based on a preset.
*   `SetChannels(int channels)`: Sets the number of audio channels. Reinitializes internal buffers if the channel count changes.
*   `SetSpeed(float speed)`: Sets the target playback speed, which affects the synthesis hop size.
*   `GetTargetSpeed()`: Gets the target playback speed.
*   `Reset()`: Resets the internal state, clearing all buffers.
*   `Process(...)`: Processes a span of input audio, writing the time-stretched result to the output span.
*   `Flush(Span<float> output)`: Flushes any remaining buffered audio data through the algorithm.
...
public class Recorder : IDisposable
{
    ...
    public Task<Result> StopRecordingAsync();
...
*   `SigningConfiguration`: Configuration for digitally signing the recorded file. If set, a detached signature file (.sig) will be generated.
*   `SampleRate`: The sample rate for recording.
*   `SampleFormat`: The sample format for recording.

**Methods:**

*   `StartRecording(SoundTags? tags = null)`: Starts the recording. Returns a `Result` indicating success or failure. Optionally accepts metadata tags to be written to the file upon completion.
*   `ResumeRecording()`: Resumes a paused recording. Returns a `Result`.
*   `PauseRecording()`: Pauses the recording. Returns a `Result`.
*   `StopRecordingAsync()`: Asynchronously stops the recording, finalizes the file, writes metadata, and generates a digital signature if configured. Returns a `Task<Result>`.
*   `StopRecording()`: Synchronously stops the recording. Returns a `Result`.
...
*   `AllPass`: An all-pass filter changes the phase of the audio signal without affecting its frequency response.
...
### Editing.Persistence `CompositionProjectManager`

```csharp
public static class CompositionProjectManager
{
    public static Task SaveProjectAsync(AudioEngine engine, Composition composition, string projectFilePath, ProjectSaveOptions? options = null, IJsonTypeInfoResolver? customTypeResolver = null);
    public static Task<(Composition Composition, List<ProjectSourceReference> UnresolvedSources)> LoadProjectAsync(AudioEngine engine, AudioFormat format, string projectFilePath, IJsonTypeInfoResolver? customTypeInfoResolver = null);
    public static Task<Result<bool>> VerifyProjectAsync(string projectFilePath, string? signatureFilePath, SignatureConfiguration config);
}
```

**Description:** Static class for saving and loading `Composition` projects to/from `.sfproj` files. Handles media consolidation, relinking, and now supports digital signing for project integrity verification. It has been updated to support NativeAOT compilation through a custom JSON type resolver.

**Methods:**
*   `SaveProjectAsync(...)`: Asynchronously saves a `Composition` to a file. Now accepts a `customTypeResolver` for AOT-compatibility with custom component types.
*   `LoadProjectAsync(...)`: Asynchronously loads a `Composition` from a file. Also accepts a `customTypeResolver` for AOT scenarios.
*   `VerifyProjectAsync(...)`: Asynchronously verifies the integrity and authenticity of a project file using a detached digital signature.

### Editing.Persistence `ProjectSaveOptions`

```csharp
public class ProjectSaveOptions
{
    public string ProjectFileVersion { get; set; }
    public string MediaSubfolderName { get; set; }
    public bool ConsolidateMedia { get; set; }
    public int SmallMediaEmbedThresholdBytes { get; set; }
    public bool EmbedSmallMedia { get; set; }
    public SignatureConfiguration? SigningConfiguration { get; set; }
}
```

**Description:** Provides configurable options for saving a project. Now includes `SigningConfiguration` to enable digital signatures on the project file.
...
public static class Log
{
    ...
    public static void Debug(string message, [CallerMemberName] string member = "", [CallerFilePath] string path = "");
    public static void Info(string message, [CallerMemberName] string member = "", [CallerFilePath] string path = "");
    public static void Warning(string message, [CallerMemberName] string member = "", [CallerFilePath] string path = "");
    public static void Error(string message, [CallerMemberName] string member = "", [CallerFilePath] string path = "");
    public static void Critical(string message, [CallerMemberName] string member = "", [CallerFilePath] string path = "");
}
```
...
*   `OnLog`: A static event that is raised whenever a log message is generated. The delegate provides the `LogEntry` struct which contains the level, message, timestamp, and caller info.
...
*   `Critical(string message, ...)`: Invokes the `OnLog` event with a `Critical` level message.

**Example Usage (in your application):**
```csharp
Log.OnLog += (entry) => {
    Console.WriteLine(entry.ToString()); // e.g., "[14:32:10] INFO MyClass.MyMethod: This is an info message"
};
```
...
```
**Description:** A standalone Biquad filter implementation. It handles state and coefficient calculation for a single audio channel based on the RBJ Audio EQ Cookbook. Used internally by `Filter` and `ParametricEqualizer`.
...
```
**Description:** A registry for resolving types by name at runtime in a NativeAOT-compatible way. This prevents the linker from trimming required types during project deserialization and avoids the use of unsafe reflection.

**Methods:**
*   `RegisterType<T>()`: Registers a custom type to ensure it can be resolved during project loading. This must be called for any user-defined Modifiers or Analyzers when running in NativeAOT.
*   `ResolveType(string typeName)`: Resolves a type by its full name from the registry or falls back to `Type.GetType` in non-AOT environments.
...
*   `Analyze(ReadOnlySpan<float> buffer, int channels)`: Analyzes the audio buffer to calculate the RMS and peak levels.
...
*   `ProcessOnAudioData(ReadOnlySpan<float> audioData)`: Processes audio data to update the level meter.
...
*   `Analyze(ReadOnlySpan<float> buffer, int channels)`: Analyzes the audio buffer to compute the frequency spectrum using an FFT.
...
*   `Dispose()`: Releases resources used by the visualizer.
*   `ProcessOnAudioData(ReadOnlySpan<float> audioData)`: Processes audio data to update the spectrum.
...
*   `Dispose()`: Releases resources used by the visualizer.
*   `ProcessOnAudioData(ReadOnlySpan<float> audioData)`: Processes audio data to update the waveform.
*   `Render(IVisualizationContext context)`: Renders the waveform visualization.


### Security `AudioEncryptor`

```csharp
public static class AudioEncryptor
{
    public static Task<string?> EncryptAsync(ISoundDataProvider source, Stream destinationStream, EncryptionConfiguration config, SignatureConfiguration? signingConfig = null, bool embedSignature = false);
    public static Task EncryptAsync(ISoundDataProvider source, string destinationPath, EncryptionConfiguration config, SignatureConfiguration? signingConfig = null, bool embedSignature = false);
    ...
    public static Task<Result<ISoundDataProvider>> VerifyAndDecryptAsync(Stream sourceStream, byte[] key, SignatureConfiguration signingConfig, string? detachedSignature = null);
    public static Task<Result<ISoundDataProvider>> VerifyAndDecryptAsync(string sourceFilePath, byte[] key, SignatureConfiguration signingConfig, string? detachedSignature = null);
}
```

**Description:** A static utility class that provides high-level, easy-to-use methods for encrypting and decrypting audio. It handles the creation and parsing of the Secure Audio Container format, streaming encryption/decryption, and integration with the digital signature system.

**Methods:**
*   `EncryptAsync(...)`: Asynchronously reads audio from an `ISoundDataProvider`, encrypts it using the provided configuration, and writes it to a stream or file. It can optionally sign the resulting container, either embedding the signature in the header or returning it as a detached string.
*   `Decrypt(...)`: Reads an encrypted audio file or stream and returns a `Result` containing an `ISoundDataProvider`. The returned provider streams the decrypted audio on-the-fly, making it suitable for direct playback without writing the entire decrypted file to disk. It takes ownership of the source stream.
*   `VerifyAndDecryptAsync(...)`: Asynchronously verifies the authenticity of an encrypted file or stream using a digital signature (either embedded or detached) *before* returning a decryption provider. This is the recommended method for authenticated decryption, as it ensures the file has not been tampered with.

### Security `FileAuthenticator`

```csharp
public static class FileAuthenticator
{
    public static Task<Result<string>> SignFileAsync(string filePath, SignatureConfiguration config);
    public static Task<Result<string>> SignStreamAsync(Stream stream, SignatureConfiguration config);

    public static Task<Result<bool>> VerifyFileAsync(string filePath, string signatureBase64, SignatureConfiguration config);
    public static Task<Result<bool>> VerifyStreamAsync(Stream stream, string signatureBase64, SignatureConfiguration config);
}
```

**Description:** A static utility class for creating and verifying ECDSA (Elliptic Curve Digital Signature Algorithm) signatures using the NIST P-384 curve. This provides strong guarantees of data integrity (the file has not been altered) and authenticity (the file was signed by the holder of the private key).

**Methods:**
*   `SignFileAsync(...)` / `SignStreamAsync(...)`: Asynchronously computes a SHA384 hash of the file/stream content and signs the hash using the private key from the `SignatureConfiguration`. Returns a `Result` containing the Base64-encoded signature string.
*   `VerifyFileAsync(...)` / `VerifyStreamAsync(...)`: Asynchronously re-computes the SHA384 hash of the file/stream, and then uses the public key from the `SignatureConfiguration` to verify that the provided signature corresponds to the computed hash. Returns a `Result` containing `true` if the signature is valid, otherwise `false`.

### Security `AudioWatermarker`

```csharp
public static class AudioWatermarker
{
    public static void EmbedOwnershipWatermark(ISoundDataProvider source, Stream destination, string text, WatermarkConfiguration config);
    public static void EmbedOwnershipWatermarkToFile(ISoundDataProvider source, string destinationPath, string text, WatermarkConfiguration config);

    public static Task<Result<string>> ExtractOwnershipWatermarkAsync(ISoundDataProvider source, WatermarkConfiguration config);
}
```

**Description:** A static utility class providing high-level methods to embed and extract robust, inaudible ownership watermarks. It uses Direct Sequence Spread Spectrum (DSSS) technology, which makes the watermark resilient to common audio processing like compression, format changes, and mild noise.

**Methods:**
*   `EmbedOwnershipWatermark(...)` / `EmbedOwnershipWatermarkToFile(...)`: Embeds a text payload into the audio from a source provider and writes the result as a 32-bit Float WAV file to the destination stream/path. This process modifies the audio data at a very low level.
*   `ExtractOwnershipWatermarkAsync(...)`: Asynchronously processes an audio stream from a source provider to search for and decode an embedded watermark. If a valid payload is found, it returns a `Result` containing the extracted text. If no watermark is detected, it returns a failure result.

### Security `AudioIdentifier`
...
public static class AudioIdentifier
{
    ...
    public static Task<Result<FingerprintResult>> IdentifyAsync(ISoundDataProvider provider, IFingerprintStore store, FingerprintConfiguration? config = null);
}
```

**Description:** A static utility class for acoustic fingerprinting. This technology allows you to identify a piece of audio by analyzing its content, similar to services like Shazam. It is robust against noise, compression, and other distortions.

**Methods:**
*   `GenerateFingerprint(...)`: Processes an entire audio provider and generates an `AudioFingerprint` object. This object contains a list of unique hashes that represent the acoustic characteristics of the audio.
*   `IdentifyAsync(...)`: Takes an unknown audio provider, generates its fingerprint, and queries an `IFingerprintStore` to find a match. It returns a `Result` of `FingerprintResult` object detailing the matched track ID, the confidence of the match, and the time offset.

### Security `WatermarkTuner`

```csharp
public static class WatermarkTuner
{
    public static Task<WatermarkConfiguration> TuneConfigurationAsync(ISoundDataProvider source, string payload, string secretKey, bool applySafetyMargin = false);
}
```

**Description:** A static utility to automatically determine the optimal watermarking configuration for a given audio source and payload. It simulates embedding and extracting at various parameters to find the best balance between robustness and inaudibility, saving developers from manual trial-and-error.

**Methods:**
*   `TuneConfigurationAsync(...)`: Asynchronously analyzes the source audio to find the most robust `WatermarkConfiguration`. It tests different spread factors and strength levels at strategic points in the audio (e.g., quiet vs. dense sections) to ensure the payload can be reliably recovered. It can apply a safety margin to the strength to account for real-world distortions.

### Security.Analyzers `ContentFingerprintAnalyzer`

```csharp
public sealed class ContentFingerprintAnalyzer : AudioAnalyzer
{
    public ContentFingerprintAnalyzer(AudioFormat format, FingerprintConfiguration? config = null);
    public IReadOnlyList<FingerprintHash> GetGeneratedHashes();
}
```

**Description:** An `AudioAnalyzer` that can be added to any `SoundComponent`'s processing chain to generate an acoustic fingerprint in real-time. It analyzes the audio stream and collects a list of `FingerprintHash` objects.

**Methods:**
*   `GetGeneratedHashes()`: Returns the complete list of hashes that have been generated from the audio processed so far.

### Security.Analyzers `IntegrityWatermarkVerifyAnalyzer`

```csharp
public sealed class IntegrityWatermarkVerifyAnalyzer : AudioAnalyzer
{
    public IntegrityWatermarkVerifyAnalyzer(AudioFormat format, WatermarkConfiguration config);
    public event Action<long>? IntegrityViolationDetected;
}
```

**Description:** An `AudioAnalyzer` used to verify the integrity of an audio stream in real-time. It expects the stream to contain a fragile watermark embedded by `IntegrityWatermarkEmbedModifier`. If it detects a broken hash chain (indicating tampering), it raises the `IntegrityViolationDetected` event.

**Events:**
*   `IntegrityViolationDetected`: Occurs when a hash mismatch is found, providing the index of the block where verification failed.

### Security.Analyzers `OwnershipWatermarkExtractAnalyzer`

```csharp
public sealed class OwnershipWatermarkExtractAnalyzer : AudioAnalyzer
{
    public OwnershipWatermarkExtractAnalyzer(AudioFormat format, WatermarkConfiguration config);
    public event Action<BitArray>? PayloadExtracted;
    public void Finish();
}
```

**Description:** An `AudioAnalyzer` that can be used to extract an ownership watermark from an audio stream in real-time. It searches for the watermark's synchronization sequence and, once found, begins decoding the payload.

**Events:**
*   `PayloadExtracted`: Occurs when the full payload has been successfully extracted as a `BitArray`.

**Methods:**
*   `Finish()`: Should be called when the audio stream ends to finalize the extraction process and invoke the `PayloadExtracted` event if a partial payload was collected.

### Security.Modifiers `StreamEncryptionModifier`
...
```
**Description:** A `SoundModifier` that performs real-time AES-256-CTR encryption or decryption on an audio stream. Because CTR mode turns the block cipher into a stream cipher, it can be used for both encryption and decryption with the same logic. It is seekable, making it suitable for use within the editing engine.

**Methods:**
*   `SeekTo(long byteOffset)`: Resets the internal cryptographic state to a specific byte offset in the stream, allowing for random access.

### Security.Modifiers `IntegrityWatermarkEmbedModifier`
...
**Description:** A `SoundModifier` that embeds a fragile integrity watermark into an audio stream. It calculates a hash of each audio block and embeds it into the Least Significant Bits (LSB) of the *next* block, creating a chain. Any modification to the audio will break this chain.

### Security.Modifiers `OwnershipWatermarkEmbedModifier`
...
**Description:** A `SoundModifier` that embeds a robust, inaudible ownership watermark into an audio stream using Direct Sequence Spread Spectrum (DSSS). It modulates the payload data onto a pseudo-random noise sequence and adds it to the audio.

### Security.Configuration `EncryptionConfiguration`
...
**Description:** Configuration settings for audio stream encryption.
*   `Key`: The encryption key. Must be 32 bytes (256 bits) for AES-256.
*   `Iv`: The initialization vector (Nonce). Must be 12 or 16 bytes for AES-CTR.
*   `EnableIntegrityCheck`: Not currently used in AES-CTR but reserved for future authenticated encryption modes.

### Security.Configuration `FingerprintConfiguration`
...
**Description:** Provides advanced configuration settings for the acoustic fingerprinting algorithm. The default values are tuned to provide a good balance of accuracy, performance, and noise resilience for identifying typical music tracks. Adjusting these parameters can optimize the algorithm for different types of audio (e.g., speech) or for different performance requirements.

**Properties:**

*   **`FftSize`**: The size of the FFT (Fast Fourier Transform) window used for spectral analysis. Must be a power of two. Larger values provide better frequency resolution but less time precision. Default is `2048`.
*   **`OverlapFactor`**: The factor by which analysis windows overlap. A value of `2` means a 50% overlap, `4` means a 75% overlap, etc. Higher overlap increases hash density and can improve matching for short clips, at the cost of increased processing time. Default is `2`.
*   **`MinFrequency`** / **`MaxFrequency`**: The frequency range (in Hz) to consider when searching for spectral peaks. The default range of `300Hz` to `5000Hz` is chosen to focus on the most perceptually significant part of the audio spectrum for music, while ignoring low-frequency rumble and high-frequency noise that are less unique.
*   **`TargetZoneSize`**: The size of the "target zone" in time (measured in analysis frames) when creating combinatorial hashes. This defines how far into the future the algorithm looks to pair an "anchor" peak with a "target" peak. A smaller value reduces hash density but also reduces the number of hashes to store and query. Default is `3`.
*   **`MinPeakMagnitude`**: An absolute threshold for the minimum magnitude a spectral peak must have to be considered. This helps filter out low-level noise. Default is `0.01`.
*   **`AdaptiveThresholdMultiplier`**: A multiplier used for adaptive peak detection. A peak is only selected if its magnitude is at least this many times greater than the local average magnitude of its frequency band. This ensures only the most prominent peaks are chosen. Default is `2.0`.
*   **`MinConfidenceThreshold`**: The minimum *absolute number* of temporally aligned hashes required to declare a successful match. This acts as a floor to prevent random, spurious alignments from being considered a match. Default is `25`.
*   **`MinRelativeConfidence`**: The minimum *relative score* (Matched Hashes / Total Query Hashes) required for a match. This is crucial for preventing false positives when fingerprinting a long audio file against a short one. It ensures that a significant *percentage* of the query's features align with the database track. A value of `0.05` means at least 5% of the query's generated hashes must match. Default is `0.05`.

### Security.Configuration `SignatureConfiguration`
...
**Description:** Configuration settings for digital signature operations.
*   `PrivateKeyPem`: The PEM-encoded Private Key (PKCS#8). Required for signing. **Should be kept secret.**
*   `PublicKeyPem`: The PEM-encoded Public Key (SubjectPublicKeyInfo). Required for verification. Safe to distribute.
*   `Generate()`: A static helper method that uses `SignatureKeyGenerator` to create a new key pair.

### Security.Configuration `WatermarkConfiguration`
...
**Description:** Configuration settings for audio watermarking.
*   `Key`: The secret key used to seed the pseudo-random number generator. Must be the same for embedding and extracting.
*   `Strength`: The embedding strength (Alpha) for ownership watermarking. Higher values are more robust but may become audible.
*   `SpreadFactor`: The chip rate for ownership watermarking. Higher values increase robustness but decrease the data rate.
*   `IntegrityBlockSize`: The block size (in samples) used for hashing in integrity watermarking.
