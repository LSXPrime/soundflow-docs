...
export const ContentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const getNavigationTree = (version: string): NavigationItem[] => {
        ...
        const categoriesMap = new Map<string, ContentPage[]>();
        ...
        const categoriesWithOrder: CategoryWithOrder[] = [];

        categoriesMap.forEach((catPages, categoryName) => {
            // Sort pages within the category by navOrder
            catPages.sort((a, b) => (a.navOrder ?? 999) - (b.navOrder ?? 999));

            // Determine the category's overall order based on its lowest navOrder item
            const minOrder = catPages.length > 0
                ? Math.min(...catPages.map(p => p.navOrder ?? 999))
                : 999;

            categoriesWithOrder.push({
                title: categoryName,
                minOrder,
        ...
        // Sort the categories themselves based on the calculated minOrder
        // If minOrder is the same, fall back to alphabetical title sorting
        ...
        // Return mapped to NavigationItem interface
        return categoriesWithOrder.map(cat => ({
            title: cat.title,
