...
4.  **Process the audio:**
    ...
    **Option A: Process All (returns `float[]`)**
    ...
    var encoder = audioEngine.CreateEncoder("cleaned_audio.wav", EncodingFormat.Wav, SampleFormat.F32, 1, 48000);
    ...
    **Option B: Process Chunks (via event or direct handler)**
    ```csharp
    var chunkEncoder = audioEngine.CreateEncoder("cleaned_audio_chunked.wav", EncodingFormat.Wav, SampleFormat.F32, 1, 48000);
